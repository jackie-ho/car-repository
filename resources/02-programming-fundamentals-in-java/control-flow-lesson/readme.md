...
Over the past few lessons, we've learned extensively about basic data types and creating methods. Now it's time to put these to use with control flow. Control flow is basically code that changes how your program acts based on certain conditions.
...
It makes the statement the ! is next to become the opposite. If something evaluates to true, it is then false. For example:

```java
int x = 5;
if(!(x > 3)){
  System.out.println("x is below 4");
}
...
String answer = (message.equals("hi")) ? "Hello!" : "Goodbye!";

System.out.println(answer);
```


## Guided Practice: Multiple Conditionals (10 mins)
...
In this case, if the first operand `string` is false, then the second operand `string.length()` will not be evaluated. The expression is basically saying "we already know the whole `&&` expression is false, because `string` is false. Why bother dealing with the second operand?"
...
Strings, as well as some other objects, have a way to compare themselves using a compareTo method call.
...
```
Say we wanted to print out the numbers 0 through 9
...
What if I wanted to print every other number between 0 and 9?
...
## Looping - Independent Practice (5 minutes)

Using the loop of your choice, print all even numbers between 0 and 50 (including 50).
